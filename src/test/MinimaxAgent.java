...
//        if (allmoves.size() == 0) {
//            Player p = getPositionsOfPiecesOutsideHome(state, currentPlayer);
...
//        System.out.println("Returning " + allMoves.size() + " number of moves for player " + currentPlayer);
        return allMoves;
    }
...
//        System.out.println("Chosen move by player " + currentPlayer + " at depth " + currentDepth + " with state value " + result.getValue());
        return result;
    }
    ...
    private State minValue(State state, int alpha, int beta, int currentDepth, Character currentPlayer, int maxDepth) throws CloneNotSupportedException {
        ...
        if (currentDepth == maxDepth) {
            ...
            //Calculating the evaluation function value of the state and set it in the variable
            state.setValue(evaluateBoardPosition(state, currentPlayer));
//            System.out.println("\t" + maxDepth + ". Min Utility State found: Value = " + state.getValue());
            return state;
        }
        currentDepth++;
        State result = (State) state.clone();
...
//        Heuristic heuristic = new Heuristic(state, currentPlayer, this.board.getCurrentPlayer());
    ...
    private int calculateMaxDepth() {
        ...
        if (GameConstants.SINGLE_MODE.equals(board.getChoice())) {
            if (board.getTimeRemaining() < 2) {
                maxDepth = 1;
            } else {
                maxDepth = 2;
            }
        } else {
            //If time remaining is less than 3 seconds then only search at depth 1
            if (board.getTimeRemaining() < 3) {
                maxDepth = 1;
            } else if (board.getTimeRemaining() < 18) {
                maxDepth = 2;
            } else {
                maxDepth = 3;
            }
